...
int main()
{
	...
	test();
	auto logger = Logger::GetInstance();
	WeatherApp app(std::make_unique<WeatherHttpPoller>(std::make_unique<JsonDataParser<WeatherData>>()), std::make_unique<DataCollectorIniReader>("../config.ini"), logger, std::make_unique<SQLiteEngine>());
	app.Run();
	return 0;
}